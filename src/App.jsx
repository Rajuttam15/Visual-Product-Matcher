...
const VisualProductMatcher = () => {
  ...
  const handleImageUpload = async (file) => {
    ...
    try {
      const uploadResponse = await fetch(
        "/.netlify/functions/imagga?path=uploads",
      ...
      const searchResponse = await fetch(
        `/.netlify/functions/imagga?path=images-similarity/fingerprints&image_upload_id=${imageHash}&limit=50`
      );
      const searchData = await searchResponse.json();

      if (!searchResponse.ok)
        throw new Error(searchData.error || "Similarity search failed");

      const formattedResults = searchData.result.distances.map((item, index) => ({
        id: item.image_id || index,
        name: `Similar Product ${index + 1}`,
        image: item.image || uploadedImage,
  ...
  const handleUrlSearch = async () => {
    ...
    try {
      ...
      if (!uploadResponse.ok) throw new Error(uploadData.error || "URL upload failed");
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50 shadow-sm transition-transform hover:scale-105"
                >
